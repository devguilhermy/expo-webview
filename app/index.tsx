...
const WEBVIEW_URL =
    ...
    'https://google.com';
const DOMAIN_NAME =
    process.env.EXPO_PUBLIC_DOMAIN_NAME || 'google.com';
...
export default function Index() {
    ...
    const [cacheTimestamp, setCacheTimestamp] = useState<
        string | null
    >(null);
    const [isSavingCache, setIsSavingCache] =
        useState(false);
    const [webErrorOffline, setWebErrorOffline] =
        useState(false); // NEW: flag for webview network error
    const [offlineError, setOfflineError] = useState<{
        code?: number;
        description?: string;
    } | null>(null);
    const [relativeTimeLabel, setRelativeTimeLabel] =
        useState<string>('');
    ...
    // Initialize Sentry context and performance monitoring
    useEffect(() => {
        const initSentry = async () => {
            ...
            setSentryTag('screen', 'webview-main');
            setSentryTag(
                'theme',
                theme.isDark ? 'dark' : 'light'
            );
            setSentryContext('webview', {
                url: WEBVIEW_URL,
                domain: DOMAIN_NAME,
            });

            addSentryBreadcrumb(
                'App Index component mounted',
                'navigation',
                'info'
            );
        };
    ...
    async function checkConnection() {
        try {
            ...
            if (loadTransaction.current) {
                const loadTime =
                    Date.now() - appStartTime.current;
                loadTransaction.current.setData(
                    'loadTime',
                    loadTime
                );
    ...
    useEffect(() => {
        (async () => {
            try {
                const html = await AsyncStorage.getItem(
                    CACHE_KEY_HTML
                );
                const ts = await AsyncStorage.getItem(
                    CACHE_KEY_TIMESTAMP
                );
    ...
    const handleWebViewMessage = (event: any) => {
        try {
            ...
            if (message === 'EXIT_APP') {
                console.log(
                    'EXIT_APP message received, attempting to exit...'
                );
                try {
                    BackHandler.exitApp();
                    console.log(
                        'BackHandler.exitApp() called successfully'
                    );
                } catch (error) {
                    console.log(
        ...
        <View style={{ flex: 1 }}>
            <WebView
                ...
                onError={(syntheticEvent) => {
                    ...
                    // Report WebView errors to Sentry
